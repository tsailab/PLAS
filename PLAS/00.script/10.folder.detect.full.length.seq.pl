...
foreach my $sub(@subs){
	...
	system("time perl 00.script/10.detect.full.length.seq.pl $blastfolder/$sub/$sub.contigs.blast.out $srcfolder/$sub\".trinity\"/Trinity.new.fasta $dbfolder/$sub/$sub.fasta $tgtfolder/$sub $sub $mode $cutoff > $tgtfolder/$sub/detect.full.length.log");
